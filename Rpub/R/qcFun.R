...
QScore <- function(expMean, obsMean, N, K=1, Qpval=0.01, C=2) {
  idNA=which(is.na(expMean))
  ss=smooth.spline(obsMean[-idNA]/N, expMean[-idNA], df=5)
  err=qnorm(1-Qpval/length(expMean)/2)*sd(ss$x)

    ##QS
  Qs=matrix(NA, length(expMean), 6)
  Qs[,1]=obsMean/N
  Qs[,2]=expMean

  for(k in 1:length(expMean)) {
    if(!is.na(Qs[k,1])) {
      pp=predict(ss, Qs[k,1])
      Qs[k,3]=ifelse(expMean[k] > pp$y+err, 1, ifelse(expMean[k] < pp$y-err, -1, 0))
      Qs[k,4]=(pp$y-expMean[k])^2
      Qs[k,5]=ifelse(Qs[k,3] == 0, 0, Qs[k,3]*(expMean[k]-(pp$y+Qs[k,3]*err))^2)
      Qs[k,6]=Qs[k,5]*abs(K-Qs[k,1])^C
    }
  }
  return(Qs)
}
...
#    dn = density(log10(as.numeric(pMat[i,!is.na(pMat[i,])])))
    dn = density(as.numeric(pMat[i,!is.na(pMat[i,])]))
    
    if(i == 1) {
      plot(dn, main="All individuals", ylim=c(0, 0.5), pch=16, cex=0.2, bty='l', xlab="", col="grey")
    } else {
...
#      dn = density(log10(as.numeric(pMat[idxC[j],!is.na(pMat[idxC[j],])])))
      dn = density(as.numeric(pMat[idxC[j],!is.na(pMat[idxC[j],])]))
      
      if(j == 1) {
        plot(dn, main=TS[i], pch=16, ylim=c(0, 0.5), cex=0.2, bty='l', xlab="", col="grey")
      } else {
        lines(dn$x,dn$y, lwd=0.2)
      }
    }
