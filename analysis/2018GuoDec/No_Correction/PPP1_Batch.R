...
####pca analysis
PCA(exDat, "prot_noQC", PPP1$PPPA_ID)
...
eve=read.table("prot_noQC.eigenvec", as.is = T)

pdf("EigenMean_noQC.pdf")
library(gplots)
...
#######Eigenvalue plot
grm=read.table(gzfile("prot_noQC.grm.gz"), as.is = T)
eva=read.table("prot_noQC.eigenval", as.is = T)

pdf("EigenVal_noQC.pdf")
