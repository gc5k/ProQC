...
for(i in 1:nrow(prot)) {
  ...
  if(TrEst[i, 1] > 10 && TrEst[i,6] > 10) {
    qclSet=qnorm(seq(1-length(clNa)/length(cl), 0.999, length.out = length(clSetSt)))
...
# plot(main=paste0("cl [", length(clSetSt), "]"), clSetSt, qclSet, cex=0.5, pch=16)
    ...
    qcsSet=qnorm(seq(1-length(csNa)/length(cs), 0.999, length.out = length(csSetSt)))
...
for(m in 1:length(ms)) {
  mat=matrix(0, nrow(prot), 4)
  idxN=which(PPP1$Tissue == "N" & PPP1$MS_ID == ms[m])
  idxT=which(PPP1$Tissue == "T" & PPP1$MS_ID == ms[m])
  for(i in 1:nrow(prot)) {
    mat[i,1]=length(which(!is.na(prot[i,idxN])))
    mat[i,2]=length(which(!is.na(prot[i,idxT])))
    protN=prot[i, idxN]
    protT=prot[i, idxT]
    mat[i,3]=mean(as.numeric(protN), na.rm = T)
    mat[i,4]=mean(as.numeric(protT), na.rm = T)
  }
  ...
  mm=mat[idxDiff,]
  rownames(mm)=row.names(prot[idxDiff,])
...
```{r, ExpCount cal}
matN=matrix(1, nrow(prot), 7)
for(i in 1:nrow(prot)) {
  iN=which(PPP1$Tissue == "N")
  iT=which(PPP1$Tissue == "T")
  matN[i,1]=length(which(!is.na(prot[i, iN])))
  matN[i,2]=mean(as.numeric(prot[i, iN]),na.rm = T)
  matN[i,3]=length(which(!is.na(prot[i, iT])))
  matN[i,4]=mean(as.numeric(prot[i, iT]), na.rm = T)
  matN[i,6]=sd(as.numeric(prot[i, iN]), na.rm = T)
  matN[i,7]=sd(as.numeric(prot[i, iT]), na.rm = T)
  if(matN[i,1] > 10 && matN[i,3]>10) {
    matN[i,5]=t.test(as.numeric(prot[i, iN]), as.numeric(prot[i, iT]))$p.value
  }
}
```
